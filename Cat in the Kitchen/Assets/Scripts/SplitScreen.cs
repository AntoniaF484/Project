...
public class SplitScreen : NetworkBehaviour
{
    ...
    public override void OnNetworkSpawn()
    {
        ...
        if (IsServer && NetworkManager.Singleton != null)
        {
            ...
            NetworkManager.Singleton.OnClientConnectedCallback +=
                OnClientChanged; //update player count when someone joins
            NetworkManager.Singleton.OnClientDisconnectCallback +=
                OnClientChanged; //update playercount when someone leaves
        }
    ...
    private void SetUpCamera(int totalPlayers)
    {
        ...
        {
            cam.rect = new Rect(0f, index == 0 ? 0.5f : 0f, 1f,
                0.5f); //horizontal split if 2 players. (index ==0?0.5f - if player index is 0, y=0.5 and player zero is in top half)
        }
        ...
        {
            cam.rect = new Rect(index % 2 * 0.5f, (index < 2) ? 0.5f : 0f, 0.5f,
                0.5f); // 2x2 grid - (X pos - index%2*0.5f remainder when/2, e.g. player 4 =index 3. 3%2 =1.5, x=0.5) y position - when index is smaller than 2, y=0.5 (top row)
        }
    }
